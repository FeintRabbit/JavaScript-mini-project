...
// prev, next & rand buttons
nextBtn.addEventListener("click", () => {
  personId++;
  if (personId > reviews.length) {
    personId = 1;
  }
  updateData();
});
prevBtn.addEventListener("click", () => {
  personId--;
  if (personId < 1) {
    personId = reviews.length;
  }
  updateData();
});
randBtn.addEventListener("click", () => {
  personId = Math.ceil(Math.random() * reviews.length);
  updateData();
});

// update DOM data function
function updateData() {
  const person = reviews.find((people) => {
    return people.id === personId;
  });
  authorImg.src = person.img;
